...
describe("Aave credit delegation", () => {
  ...
  const itSuccessfullyDelegatesWith = ({
    ...
    describe(`when using ${ethers.utils.formatEther(
      depositAmount
    )} ${depositAsset} as collateral to delegate ${ethers.utils.formatEther(
      delegatedAmount
    )} ${loanAsset}, with ${variable ? "variable" : "stable"} interest`, () => {
      ...
      before(
        ...
        async function () {
          ...
          if (!assetData.usageAsCollateralEnabled) {
            console.log(
              yellow(
                `      > ${depositAsset} is not enabled for use as collateral`
              )
            );
          ...
          if (!assetData.borrowingEnabled) {
            console.log(
              yellow(`      > ${loanAsset} is not enabled for borrowing`)
            );
            canBeBorrowed = false;
          }
          if (!variable && !assetData.stableBorrowRateEnabled) {
            console.log(
              yellow(`      > ${loanAsset} is not enabled for stable borrowing`)
            );
            canBeBorrowed = false;
          }
          if (!assetData.isActive) {
          ...
          console.log(
            gray(
              `      > ${loanAsset} liquidity: ${ethers.utils.formatEther(
          ...
          if (assetData.availableLiquidity.lt(delegatedAmount)) {
            console.log(
              yellow(`      > liquidity for ${loanAsset} is not sufficient`)
            );
            canBeBorrowed = false;
          }
          ...
          if (!canBeUsedAsCollateral || !canBeBorrowed) {
            console.log(
              yellow(
                `      > the ${depositAsset} / ${loanAsset} pair is not available for credit delegation, skipping tests...`
              )
            );
      ...
      describe(`when the lender has ${ethers.utils.formatEther(
        depositAmount
      )} ${depositAsset}`, () => {
        before("transfer asset from a large holder to the lender", async () => {
          const holder = await impersonateAddress(
            addresses.holders[depositAsset]
          );
          const holderBalance = await depositToken.balanceOf(holder.address);
          if (holderBalance.lt(depositAmount)) {
            throw new Error(
              `Holder only has ${ethers.utils.formatEther(
                holderBalance
              )} ${depositAsset}`
            );
          }
          ...
          await depositToken.transfer(
            lender.address,
            await depositToken.balanceOf(holder.address)
          );
        });

        it("shows that the lender has the necessary balance", async () => {
          expect(await depositToken.balanceOf(lender.address)).to.be.gte(
            depositAmount
          );
        });
        ...
        before("the lender deposits collateral", async () => {
          it("shows that the lender has no collateral", async () => {
            const userData = await lendingPool.getUserAccountData(
              lender.address
            );
        ...
        describe(`when the lender approves the pool to spend its ${depositAsset}`, () => {
          ...
          describe(`(1) when the lender deposits ${ethers.utils.formatEther(
            depositAmount
          )} ${depositAsset} as collateral`, () => {
            ...
            describe("(2) when the lender connects to the debt token", async () => {
              before("connect to debt token", async () => {
                ...
                const debtTokenAddress = variable
              ...
              it("shows that the associated debt tokens asset is correct", async () => {
                expect(await debtToken.UNDERLYING_ASSET_ADDRESS()).to.be.equal(
                  loanAssetAddress
                );
              });
              ...
              describe("before the lender approves credit delegation", () => {
                ...
                it("shows that the lender has no debt", async () => {
                  ...
                  expect(userData.totalDebtETH).to.be.lt(
                ...
                describe("when the lender withdraws collateral before approving credit", () => {
                  ...
                  before("record current values", async () => {
                    balanceBefore = await depositToken.balanceOf(
                      lender.address
                    );
                  });
                  ...
                  before("withdraw collateral", async () => {
                    ...
                    await lendingPool.withdraw(
                      depositAssetAddress,
                      depositAmount.div(ethers.utils.parseEther("2")),
                      lender.address
                    );
                  });
              ...
              describe(`(3) when the lender approves ${ethers.utils.formatEther(
                delegatedAmount
              )} ${loanAsset} of credit to the borrower`, () => {
                ...
                describe(`(4) when the borrower borrows ${ethers.utils.formatEther(
                  delegatedAmount
                )} ${loanAsset}`, () => {
                  ...
                  describe(`when the borrower approves the pool to spend its ${loanAsset}`, () => {
                    ...
                    describe("when the borrower repays 50% of the loan", () => {
                      ...
                      before("record the initial values", async () => {
                        ...
                        balanceBefore = await loanToken.balanceOf(
                          borrower.address
                        );
                      });
                      ...
                      it("reduces the lenders debt", async () => {
                        ...
                        expect(userData.totalDebtETH).to.be.lt(debtBefore);
                      });
                      ...
                      describe("(5) when the borrower repays the entire loan", () => {
                        ...
                        describe("(6) when the lender withdraws collateral", () => {
                          ...
                          before(
                            "lender withdraws all collateral",
                            async () => {
                              lendingPool = lendingPool.connect(lender);

                              amountToWithdraw = depositAmount.sub(
